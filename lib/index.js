...
module.exports = function patchBluebird(ns, Promise) {
    ...
    // `Promise.join` sometimes call the callback synchronously so needs special shim.
    shimJoin(Promise, ns);
    ...
    if (v3) {
        // In bluebird v3, `Promise.map`, `.map`, `Promise.filter` and `.filter`
