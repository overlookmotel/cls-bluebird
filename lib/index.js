...
module.exports = function patchBluebird(ns, Promise) {
	 ...
	 *   Promise.race() / .race()
	 *
	 * NB In bluebird v3 due to bug https://github.com/petkaantonov/bluebird/issues/1148
	 * `Promise.map()`, `.map()`, `Promise.filter()` and `.filter()` call the callback synchronously
	 * if input array contains only values or resolved promises, but async if any promises are pending.
	 * So handler is sometimes bound to CLS context unnecessarily, but this does no harm
	 * beyond the very slight performance overhead of an extra `ns.bind()` call.
	 */
	...
	// In bluebird v3, `.each()` calls `.mapSeries()` immediately which binds callback.
	// `Promise.each()` static method still requires patching in bluebird v3.
	if (v3) {
	 ...
	 *
	 * NB In bluebird v3 so `onCancel` handler will be called
	 ...
	 * TODO Bind `options.yieldHandler` somehow?
